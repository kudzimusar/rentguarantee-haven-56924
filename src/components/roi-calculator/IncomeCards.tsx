...
interface IncomeCardsProps {
  properTierMonthlyIncome: number;
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
      <Card className="border-green-100">
        <CardContent className="p-4">
          <h4 className="text-base font-semibold text-green-700 mb-1">With Proper Tier</h4>
          <p className="text-2xl font-bold text-green-800">{formatCurrency(properTierMonthlyIncome)}</p>
          <p className="text-xs text-gray-600">Monthly Effective Income</p>
          <p className="text-sm mt-2 text-green-700">Annual: {formatCurrency(properTierNetIncome)}</p>
        </CardContent>
      </Card>
